...
# Launch counter
url = "https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Flocalhost%2FALTS&count_bg=%23FF0000&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=Launches&edge_flat=false"
...
def ALTS(q, text, indent):
    if q:
        printable = (Fore.BLUE + "[Question] " + Style.RESET_ALL + Style.DIM + text + Style.RESET_ALL)
    else:
        printable = (Fore.MAGENTA + "[ALTS] " + Style.RESET_ALL + Style.DIM + text + Style.RESET_ALL)
    if indent:
        printable = printable + "\n"
    return printable
...
def clearfig():
    ...
    result = pyfiglet.figlet_format("A L T S", font="alligator2")
    print("\u001b[35m" + result.rstrip() + "\u001b[0m \n")
    print(ALTS(False, "Welcome to ALTS [" +  local_version + "]", False))
    print(ALTS(False, "Join the discord: discord.gg/alts-on-top", True))

try:

    os.system('cls' if os.name == 'nt' else 'clear')

    typewriter(ALTS(False, "Loading...", True), "print")

    result = pyfiglet.figlet_format("A L T S", font="alligator2")
    typewriter("\u001b[35m" + result.rstrip() + "\u001b[0m \n", "logo")
    typewriter("\n" + ALTS(False, "Loaded", True), "print")
    ...
    option = True if typewriter(ALTS(True, "Do you want aimbot or a triggerbot? (1/2): ", False), "input").lower() == "1" else False

    if option:

        clearfig()
        show_frame = True if typewriter(ALTS(True, "Do you want to use a GUI? (y/n): ", False), "input").lower() == "y" else False
        ...
        if os.path.exists(config_file_path):
            config = True if typewriter(ALTS(True, "Do you want to use a config? (y/n): ", False), "input").lower() == "y" else False
        else:
            exit
        ...
        if config:
            ...
            if show_frame:
                floating = True if typewriter(ALTS(True, "Do you want the detection window to be pinned on top? (y/n): ", False), "input").lower() == "y" else False
            else:
                floating = False

            clearfig()

            shoot = True if typewriter(ALTS(True, "Do you want it to shoot? (y/n): ", False), "input").lower() == "y" else False
            clearfig()

            key = typewriter(ALTS(True, "Press the key you want to use to aim: ", False), "input").lower()
            clearfig()

            placement_side = typewriter(ALTS(True, "Enter 'left' or 'right' or 'no' to pick a detection block: ", False), "input").lower()
            clearfig()

            smoothness = typewriter(ALTS(True, "Smoothness? (1-10): ", False), "input")
            clearfig()
            smoothness = int(smoothness)

            save_config = True if typewriter(ALTS(True, "Do you want to save this config? (y/n): ", False), "input").lower() == "y" else False
            clearfig()
            if save_config:
                config_data = {
                    "floating": floating,
                ...
                with open('config.json', 'w') as f:
                    ...
                    typewriter(ALTS(False,"Config file saved.", False), "print")
            else:
                typewriter(ALTS(False, "Config file not saved.", False), "print")
        ...
        typewriter(ALTS(False, f"Hold {key} for it to aim.", True), "print")
        typewriter(ALTS(False, Style.RESET_ALL + "\u001b[32mRunning...\u001b[0m", False), "print")
        ...
        while True:
            ...
            if placement_side == 'left':
                ...
                typewriter(ALTS(False, "Invalid input. Please enter 'left' or 'no'.", False), "print")
                exit(1)
        ...
        clearfig()
        key = typewriter(ALTS(True, "Enter the key you want to use to activate the triggerbot: ", False), "input")
        clearfig()
        delay = int(typewriter(ALTS(True, "Enter the delay (ms) you want to use for the triggerbot: ", False), "input"))

        clearfig()
        typewriter(ALTS(False, f"Hold {key} for it to shoot when it notices changes.", True), "print")
        typewriter(ALTS(False, "\u001b[32mRunning...\u001b[0m", False), "print")
    ...
    clearfig()
    typewriter(ALTS(False, "\u001b[0m\033[91mExiting... Goodbye!\n\u001b[0m", False), "print")
    time.sleep(0.2)
